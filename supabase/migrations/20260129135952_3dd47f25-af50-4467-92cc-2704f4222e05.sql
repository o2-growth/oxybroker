-- 1. Criar função RPC para buscar maior lance do usuário em um lote
CREATE OR REPLACE FUNCTION public.get_user_max_bid_on_lot(_lot_id uuid)
RETURNS numeric
LANGUAGE sql
STABLE
SECURITY DEFINER
...
-- 2. Atualizar função place_bid_atomic para validar saldo pelo incremento
...
BEGIN
  ...
  IF v_wallet.balance IS NULL OR v_wallet.balance < v_required_balance THEN
    RETURN jsonb_build_object(
      'error_code', 'INSUFFICIENT_BALANCE',
      'error_message', format('Saldo insuficiente. Necessário: R$ %s. Seu saldo: R$ %s', 
        to_char(v_required_balance, 'FM999G999D00'),
...
-- 3. Atualizar função close_auction_atomic com fallback para próximos lances
